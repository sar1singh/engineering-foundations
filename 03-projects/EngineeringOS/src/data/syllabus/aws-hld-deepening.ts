...
export const awsHldDeepeningTopics: SyllabusTopic[] = [
  ...
  topic(17, "ci-cd-blue-green-canary", "CI/CD Blue-Green and Canary", "CI/CD blue-green and canary deployment releases changes through automated checks, traffic shifting, health monitoring, and rollback.", "Deploy small, observe honestly, promote only when health is proven.", "Use pipelines, immutable artifacts, deployment roles, weighted traffic, alarms, and rollback runbooks to reduce release risk.", "GitHub Actions/CodePipeline builds artifact; CodeDeploy or weighted routing shifts 10% traffic; CloudWatch alarms stop promotion."),
  topic(18, "kms", "KMS", "AWS KMS manages encryption keys used to protect data across AWS services and applications.", "Keys are security boundaries and audit objects.", "Know envelope encryption, key policies, grants, rotation, multi-region keys, and service integration.", "S3/RDS/DynamoDB encrypted with KMS CMK; CloudTrail records key use; IAM/key policy controls access."),
  topic(19, "cloudtrail", "CloudTrail", "CloudTrail records AWS account API activity for audit, security, and incident investigation.", "Every control-plane action should leave an audit trail.", "Use organization trails, S3 log archive, CloudWatch/EventBridge alerts, and tamper-resistant retention.", "CloudTrail -> central S3 log bucket -> EventBridge alert for sensitive API actions."),
  topic(20, "backup-dr", "Backup and DR", "Backup and DR design defines how data and workloads are restored after accidental deletion, corruption, or regional failure.", "Know RPO/RTO and test restores before disaster.", "Discuss backups, snapshots, cross-region copy, pilot light, warm standby, active-active, and restore drills.", "AWS Backup policies protect RDS/EBS/EFS; cross-region copies support DR; runbooks test restore time."),
  topic(21, "cost-optimization", "Cost Optimization", "Cost optimization delivers required business value at the lowest sustainable AWS cost.", "Architecture choices are also cost choices.", "Know right-sizing, autoscaling, storage classes, reserved/savings plans, caching, data transfer, and observability for cost anomalies.", "Use budgets/anomaly detection; S3 lifecycle; Graviton/right-size EC2; avoid NAT/data-transfer surprises.")
];
