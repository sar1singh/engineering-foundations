...
export const hldCaseStudyTopics: SyllabusTopic[] = [
  ...
  hldCase({ order: 7, slug: "hld-search-autocomplete", title: "Search and Autocomplete HLD", definition: "Design search and autocomplete with indexing, ranking, freshness, typo tolerance, and low-latency suggestions.", mentalModel: "Source data feeds an index; reads optimize relevance and latency.", theory: "Cover ingestion, indexing, query path, autocomplete, ranking, freshness lag, abuse limits, and reindexing.", awsVariant: "Catalog source -> Kinesis/SQS indexing -> OpenSearch -> ElastiCache hot suggestions -> API Gateway/ECS query API -> CloudWatch search dashboards." }),
  hldCase({ order: 8, slug: "hld-file-storage", title: "File Storage HLD", definition: "Design file upload, metadata, permissions, sharing, previews, versioning, and scanning.", mentalModel: "Objects live in blob storage; metadata and permissions control safe access.", theory: "Cover pre-signed uploads, metadata, access control, preview generation, virus scanning, versioning, and retention.", awsVariant: "S3 + CloudFront signed URLs -> API metadata service -> RDS/DynamoDB metadata -> SQS/Lambda preview and scan workers -> KMS encryption." }),
  hldCase({ order: 9, slug: "hld-metrics-observability", title: "Metrics and Observability HLD", definition: "Design telemetry ingestion, dashboards, alerts, retention, and tenant-isolated querying.", mentalModel: "High-volume events become queryable signals and alerts.", theory: "Cover collectors, ingestion queues, stream processors, time-series/log stores, alert evaluation, retention, and backpressure.", awsVariant: "Kinesis/MSK ingestion -> Lambda/ECS processors -> S3/OpenSearch/Timestream stores -> API query layer -> CloudWatch dashboards and alarms." }),
  hldCase({ order: 10, slug: "hld-ecommerce-checkout", title: "Ecommerce Checkout HLD", definition: "Design cart, pricing, inventory reservation, payment, order creation, and fulfillment events.", mentalModel: "Checkout is a saga around inventory, money, and order state.", theory: "Cover cart state, pricing/coupons, stock reservation, payment idempotency, order state, fulfillment events, and compensation.", awsVariant: "API Gateway/ECS services -> DynamoDB carts -> RDS inventory/orders -> Step Functions checkout saga -> SQS/EventBridge fulfillment events -> KMS/CloudTrail audit." }),
  hldCase({ order: 11, slug: "hld-ride-sharing", title: "Ride Sharing HLD", definition: "Design rider-driver matching, location updates, trip lifecycle, pricing, payment, notifications, and safety flows.", mentalModel: "Realtime location feeds drive matching; trip state protects money and safety.", theory: "Cover geospatial indexing, location freshness, matching, surge/pricing, trip states, cancellation, payment, safety, and observability.", awsVariant: "API Gateway/ECS -> Kinesis location stream -> DynamoDB geohash partitions -> ElastiCache hot availability -> Step Functions trip/payment flow -> CloudWatch matching metrics." }),
  hldCase({ order: 12, slug: "hld-video-streaming", title: "Video Streaming HLD", definition: "Design upload, transcoding, metadata, CDN playback, DRM/security, analytics, and cost-aware storage.", mentalModel: "Prepare media once, deliver many times from the edge.", theory: "Cover upload, transcoding queues, adaptive bitrate renditions, metadata, CloudFront/CDN delivery, signed URLs, playback metrics, and storage lifecycle.", awsVariant: "S3 upload -> Step Functions transcoding workflow -> media workers -> CloudFront signed playback -> DynamoDB/RDS metadata -> Kinesis analytics -> S3 lifecycle policies." }),
  hldCase({ order: 13, slug: "hld-distributed-rate-limiter", title: "Distributed Rate Limiter HLD", definition: "Design low-latency request limiting across many app instances, tenants, regions, and abuse patterns.", mentalModel: "A global limiter is policy plus fast shared counters plus safe degradation.", theory: "Cover token bucket/sliding window, tenant policies, Redis counters, regional limits, API Gateway/WAF coarse controls, fallback behavior, and audit metrics.", awsVariant: "API Gateway/WAF coarse limits -> ECS/EKS limiter service -> ElastiCache Redis counters -> DynamoDB policy store -> CloudWatch metrics -> SQS audit stream." })
];
